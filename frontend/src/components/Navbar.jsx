  ...
  return (
    <nav style={s.nav}>
      ...
      <div style={s.links}>
        {user ? (
          <>
            <Link to="/marketplace" style={s.link}>{t('nav.browse')}</Link>
            {user.role === 'farmer' && <Link to="/dashboard" style={s.link}>{t('nav.dashboard')}</Link>}
            {user.role === 'buyer' && <Link to="/orders" style={s.link}>{t('nav.orders')}</Link>}
            {user.role === 'buyer' && <Link to="/addresses" style={s.link}>{t('nav.addresses')}</Link>}
            {user.role === 'admin' && <Link to="/admin" style={{ ...s.link, color: '#ffeaa7' }}>{t('nav.admin')}</Link>}
            {user.role !== 'admin' && <Link to="/wallet" style={s.link}>{t('nav.wallet')}</Link>}
