...
export default function Wallet() {
  ...
  return (
    <div style={s.page}>
      <div style={s.title}>{t('wallet.title')}</div>
      ...
      {disclaimerVisible && (
        <div style={s.disclaimer} role="alert" aria-label="Testnet disclaimer">
          ...
          <div style={s.disclaimerBody}>
            <div style={s.disclaimerTitle}>{t('wallet.testnetTitle')}</div>
            {t('wallet.testnetBody')}
          </div>
        ...
        <>
          <div style={s.card}>
            ...
            <div style={{ fontSize: 12, color: '#888', marginTop: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ background: '#fff3cd', color: '#856404', border: '1px solid #ffc107', borderRadius: 4, padding: '1px 7px', fontWeight: 600, fontSize: 11 }}>
                {t('wallet.testnetBadge')}
              </span>
              {t('wallet.noRealValue')}
            </div>
          ...
          <div style={s.card}>
            <h3 style={{ marginBottom: 4, color: '#333' }}>{t('wallet.referralTitle')}</h3>
            <p style={{ fontSize: 13, color: '#888', marginBottom: 16 }}>{t('wallet.referralDesc')}</p>
            <div style={{ background: '#f8f9fa', padding: 16, borderRadius: 8, border: '1px dashed #ced4da' }}>
              <div style={{ fontSize: 12, color: '#6c757d', marginBottom: 4, textTransform: 'uppercase', fontWeight: 600 }}>
              ...
              <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                ...
                <button onClick={() => { navigator.clipboard.writeText(wallet?.referralCode); alert(t('wallet.codeCopied')); }}
              ...
              <div style={{ marginTop: 16 }}>
                <button onClick={() => { const link = `${window.location.origin}/register?ref=${wallet?.referralCode}`; navigator.clipboard.writeText(link); alert(t('wallet.referralLinkCopied')); }}
          ...
          <div style={s.card}>
            <h3 style={{ marginBottom: 4, color: '#333' }}>{t('wallet.sendTitle')}</h3>
            <p style={{ fontSize: 13, color: '#888', marginBottom: 8 }}>{t('wallet.sendDesc')}</p>
            <form onSubmit={handleSend} noValidate>
              <label style={s.label}>{t('wallet.destination')}</label>
              <input style={s.input} type="text" placeholder={t('wallet.destinationPlaceholder')}
                value={sendForm.destination} onChange={e => setSendForm(f => ({ ...f, destination: e.target.value }))} spellCheck={false} />
              <div style={{ display: 'flex', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  <label style={s.label}>{t('wallet.amount')}</label>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <input style={{ ...s.input, flex: 1 }} type="number" min="0.0000001" step="any" placeholder="0.00"
                      value={sendForm.amount} onChange={e => setSendForm(f => ({ ...f, amount: e.target.value }))} />
                    <select style={{ padding: '9px 10px', border: '1px solid #ddd', borderRadius: 8, fontSize: 14, background: '#f9f9f9', cursor: 'pointer', minWidth: 90 }}
                      value={sendForm.currency} onChange={e => setSendForm(f => ({ ...f, currency: e.target.value }))} aria-label={t('wallet.currency')}>
                  ...
                  {sendForm.currency !== 'XLM' && (
                    <div style={{ marginTop: 6, fontSize: 12, color: '#c0392b', display: 'flex', alignItems: 'center', gap: 4 }}>
                      {t('wallet.onlyXlmWarning')}
                    </div>
                  )}
                  {sendForm.currency === 'XLM' && (
                    <div style={{ marginTop: 5, fontSize: 11, color: '#888' }}>{t('wallet.onlyXlm')}</div>
                  )}
                </div>
                <div style={{ flex: 1 }}>
                  <label style={s.label}>{t('wallet.memo')} <span style={{ color: '#aaa', fontWeight: 400 }}>{t('wallet.memoHint')}</span></label>
                  <input style={s.input} type="text" maxLength={28} placeholder={t('wallet.memoPh')}
                    value={sendForm.memo} onChange={e => setSendForm(f => ({ ...f, memo: e.target.value }))} />
                </div>
              </div>
              <button type="submit" style={{ ...s.btn, marginTop: 16 }} disabled={sending}>
                {sending ? t('wallet.sending') : t('wallet.sendBtn')}
              </button>
            </form>
            {sendMsg && (
              <div style={{ ...s.msg, background: sendMsg.type === 'ok' ? '#d8f3dc' : '#fee', color: sendMsg.type === 'ok' ? '#2d6a4f' : '#c0392b' }}>
                ...
                {sendMsg.txHash && (
                  <div style={{ marginTop: 6, fontSize: 12 }}>
                    TX: <a href={`https://stellar.expert/explorer/testnet/tx/${sendMsg.txHash}`} target="_blank" rel="noreferrer"
                      style={{ color: '#2d6a4f', wordBreak: 'break-all' }}>{sendMsg.txHash}</a>
                  </div>
                )}
              </div>
            )}
          ...
          <div style={s.card}>
            <h3 style={{ marginBottom: 16, color: '#333' }}>{t('wallet.txHistory')}</h3>
            {txs.length === 0 && <p style={{ color: '#888', fontSize: 14 }}>{t('wallet.noTxs')}</p>}
            {txs.map(tx => (
              <div key={tx.id} style={s.tx}>
                <div>
                  <div style={tx.type === 'sent' ? s.sent : s.recv}>
                    {tx.type === 'sent' ? t('wallet.sent') : t('wallet.received')} {parseFloat(tx.amount).toFixed(2)} XLM
                  </div>
                  <div style={{ fontSize: 12, color: '#888' }}>{new Date(tx.created_at).toLocaleString()}</div>
                  <div style={s.hash}>{tx.transaction_hash}</div>
                </div>
                <a href={`https://stellar.expert/explorer/testnet/tx/${tx.transaction_hash}`} target="_blank" rel="noreferrer"
                  style={{ fontSize: 12, color: '#2d6a4f' }}>{t('wallet.view')}</a>
              </div>
            ))}
          </div>
        </>
