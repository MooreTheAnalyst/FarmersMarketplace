...
beforeEach(() => {
  ...
  mockDb.prepare = jest.fn(() => ({ get: jest.fn(), all: jest.fn().mockReturnValue([]), run: jest.fn().mockReturnValue({ lastInsertRowid: 1, changes: 1 }) }));
  mockDb.exec = jest.fn();
  mockDb.transaction = jest.fn((fn) => (...args) => fn(...args));
